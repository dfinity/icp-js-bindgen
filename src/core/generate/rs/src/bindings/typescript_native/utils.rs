...
/// together, once per module, in the sorted order of the candid ids, each stepping past the
/// names the module binds itself, every type's identifier and every local allocated before it.
///
/// The names taken from the file — the actor class and its interface — are not among those,
/// and neither are the enums named after inline variant tags, so a local can still land on
/// one of them. The module checks refuse that module rather than the generator naming around
/// it.
...
mod tests {
    ...
    fn import_locals_never_collide_with_each_other() {
        ...
        let table = ImportLocals::new(&env);
