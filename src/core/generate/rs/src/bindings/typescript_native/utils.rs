...
/// the module — and renaming one would change the generated client's public API while the
/// wire call kept the candid name.
...
mod sanitizer_tests {
    ...
    /// puts a quoted name inside an `Ident`, which is what the two used to share.
    #[test]
