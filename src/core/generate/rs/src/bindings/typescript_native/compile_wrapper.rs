...
pub fn compile_wrapper(
    ...
    if actor.is_some() {
        add_create_actor_exports(&mut module, service_name, env);
    }
...
fn wrapper_actor_implementation(
    ...
    match actor_type.as_ref() {
        ...
        TypeInner::Class(_, t) => {
            wrapper_actor_implementation(env, module, t, syntax, service_name, converter, span)
        }
        _ => {}
    }
}
...
fn wrapper_actor_var(
    ...
    interface_actor_var(env, module, type_id, service_name, span);
    ...
    let serv = match type_ref.as_ref() {
        ...
        TypeInner::Var(var_id) => {
            ...
            if let TypeInner::Service(serv) = referred_type.as_ref() {
                serv
            } else {
                return;
            }
...
fn create_actor_class(
    ...
    let methods: Vec<ClassMember> = serv
        ...
        .filter_map(|(method_id, func_ty)| {
            if let TypeInner::Func(func) = func_ty.as_ref() {
                Some(create_actor_method(env, method_id, func, converter))
            } else if let TypeInner::Var(inner_id) = func_ty.as_ref() {
    ...
    ClassDecl {
        ident: get_ident(&service_class_name(service_name, env)),
        declare: false,
        class: Box::new(Class {
            span: DUMMY_SP,
            body: class_body_members,
            super_class: None,
            type_params: None,
...
fn add_create_actor_exports(module: &mut Module, service_name: &str, env: &TypeEnv) {
    ...
    let create_actor_function = create_actor_function(service_name, env);
...
fn create_actor_function(service_name: &str, env: &TypeEnv) -> FnDecl {
    let span = DUMMY_SP;

    // The same value annotates the return type and is constructed in the body, so the two
    // cannot drift apart.
    let class_name = service_class_name(service_name, env);
