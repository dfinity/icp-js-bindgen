 ...
 * type references — against being quoted rather than sanitized. Quoting is correct in
 * property position (`'my-field': bigint` is legal) and a syntax error in binding position
 * (`interface 'my-field'` is not), and the two used to share a code path.
 *
 * The generated wrapper ships with a `// @ts-nocheck` header, which suppresses type errors
 * but not parse errors. The header is stripped here and the file is parsed.
 */
...
/**
 * Deliberately *looser* than the generator's own rule, so this checks "TypeScript would
 * accept this" rather than restating the implementation. Modelled on ECMAScript
 * `IdentifierName`, omitting the zero-width joiners and escape sequences the grammar permits
 * but the generator never emits. The generator restricts itself to `XID_*`, a subset — so its
 * output always satisfies this, and a regression that emitted a quoted name is still caught.
 */
const ENFORCED_IDENTIFIER_SHAPE = /^[\p{ID_Start}$_][\p{ID_Continue}$]*$/u;

/** Every declaration kind the generator emits puts its name in binding position. */
const DECLARED_IDENTIFIER =
  /^\s*(?:export\s+)?(?:declare\s+)?(?:class|interface|enum|type)\s+(\S+?)(?:<|\s|=|$)/gm;

/** Basenames that are not legal identifiers, or that become a global once capitalized. */
...
const ALL = [...AWKWARD_SERVICE_NAMES, ...AWKWARD_CANDID_NAMES];
...
describe('identifiers in binding position', () => {
  it.each(ALL)('emits parseable TypeScript for %s', async (serviceName) => {
  ...
  it.each(ALL)('declares only legal identifiers for %s', async (serviceName) => {
    await generateFixture(serviceName);
    for (const file of [`${serviceName}.ts`, `${serviceName}.d.ts`]) {
      const source = await readFileFromOutput(file);
  ...
  // The specifier used to rewrite `-` to `_` while the file kept the dash, so it pointed at a
  // module that was never written. Parsing the specifier out of the generated file rather
  // than reconstructing it from the service name is what makes this a real check —
  // reconstructing it would assume exactly what the bug got wrong.
  it.each(AWKWARD_SERVICE_NAMES)('imports declarations that exist for %s', async (serviceName) => {
    ...
    for (const file of [`${serviceName}.ts`, `${serviceName}.d.ts`]) {
      ...
      for (const specifier of specifiers) {
        // Imports are extensionless, so any declaration extension may satisfy one.
  ...
  it('sanitizes an enum name derived from candid tags', async () => {
    ...
    // The tag survives verbatim as an enum *member* — quoting is legal there — while the enum
    // name, which is an identifier, is sanitized.
...
/**
 * Parse-level diagnostics only. `transpileModule` does not resolve modules or check types, so
 * this reports malformed syntax without needing the `@icp-sdk/core` packages present.
 */
