 ...
 * `IdentifierName`, omitting only the escape sequences the grammar permits and the generator
 * never emits. ZWNJ and ZWJ are included because `IdentifierPart` accepts them and the
 * generator preserves them, which `\p{ID_Continue}` alone would reject.
 */
...
describe('identifiers in binding position', () => {
  ...
  // The specifier must name the file the JavaScript layer actually writes. Parsing it out of
  // the generated file rather than reconstructing it from the service name is what makes this
  // a real check — reconstructing it would assume the very thing under test.
