...
describe("generate", () => {
  ...
  it("should generate a bindgen with canister env feature", async () => {
    ...
    await expectGeneratedOutput(SNAPSHOTS_DIR, helloWorldServiceName);
    await expect(
      await readFileFromOutput("canister-env.d.ts")
    ).toMatchFileSnapshot(
...
async function expectGeneratedOutput(
  snapshotsDir: string,
  serviceName: string
): Promise<void> {
  const generatedOutputDir = join(snapshotsDir, serviceName);
  const generatedOutputDeclarationsDir = join(
    generatedOutputDir,
    "declarations"
  );

  const declarationsJs = await readFileFromOutput(
    `declarations/${serviceName}.did.js`
  );
  await expect(declarationsJs).toMatchFileSnapshot(
    `${generatedOutputDeclarationsDir}/${serviceName}.did.js.snapshot`
  );

  const declarationsTs = await readFileFromOutput(
    `declarations/${serviceName}.did.d.ts`
  );
  ...
  await expect(serviceTs).toMatchFileSnapshot(
    `${generatedOutputDir}/${serviceName}.ts.snapshot`
  );

  const indexTs = await readFileFromOutput("index.ts");
  await expect(indexTs).toMatchFileSnapshot(
    `${generatedOutputDir}/index.ts.snapshot`
  );
}
